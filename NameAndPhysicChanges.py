...
def instruction():
    instr_label=label(pos=(0, 0, 0), text='Use mouse to control your platform',
...
def instruction1():
    instr_label=label(pos=(0, 0, 0), text='Use keyboard to control your platform',
                      height=25, border=50, color=(0, 1, 1), opacity=0.5)
    sleep(3)
    instr_label.visible=0
    del instr_label
...
def New_Game_Keyboard():
    ...
    ball = sphere( pos=vector(0,0,0), velocity=vector(-0.15, -0.23, +0.27),
                   color=(0, 1, 1), radius = 1,)
...
def Menu():
    ...
    m_mouse_button = wx.Button(pu, label='Mouse', pos=(369,225))
    m_mouse_button.Bind(wx.EVT_BUTTON, m_mouse)
    
    m_keyboard_button = wx.Button(pu, label='Keyboard', pos=(492,225))
